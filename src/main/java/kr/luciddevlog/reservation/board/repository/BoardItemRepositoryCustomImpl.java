...
public class BoardItemRepositoryCustomImpl implements BoardItemRepositoryCustom{
    ...
    public Page<BoardItemWithAuthorName> findBoardItemsByCategory(BoardCategory category, Pageable pageable) {
        JPAQuery<BoardItem> query = jpaQueryFactory
        ...
        List<BoardItem> boardItems = query
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .fetch();

        List<BoardItemWithAuthorName> content = boardItems.stream()
                .map(item -> new BoardItemWithAuthorName(
                        item.getId(),
                        item.getTitle(),
                        item.getContent(),
                        item.getCreatedAt(),
